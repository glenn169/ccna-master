import { BarChart3, ClipboardCheck, Cloud, CloudOff, FlaskConical, GraduationCap, Home, Menu, Network, Sparkles, User, X } from 'lucide-react'
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[260px_1fr]">
    ...
    <div className="min-w-0"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:px-8"><div className="lg:hidden"><Brand dark /></div><button className="rounded-lg p-2 lg:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button><div className="ml-auto flex items-center gap-2 sm:gap-3"><span className="hidden rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-700 sm:inline">{summary.streakDays} day streak</span><NavLink to="/account" className={`flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-black ${user ? syncError ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>{user ? syncError ? <CloudOff size={17}/> : <Cloud size={17} className={syncing ? 'animate-pulse' : ''}/> : <User size={17}/>}<span className="hidden sm:inline">{user ? syncing ? 'Syncing' : 'Synced' : 'Sign in'}</span></NavLink></div></header>
