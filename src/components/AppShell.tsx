...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[260px_1fr]">
    ...
    <div className="min-w-0"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:px-8"><div className="lg:hidden"><Brand dark /></div><button className="rounded-lg p-2 lg:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button><div className="ml-auto hidden items-center gap-3 sm:flex"><span className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-700">{summary.streakDays} day streak</span><div className="grid h-9 w-9 place-items-center rounded-full bg-navy-900 text-sm font-bold text-white">GM</div></div></header>
