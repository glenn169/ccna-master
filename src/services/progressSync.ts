...
import { db, emptyProgress, type ExamAttempt, type LabProgress, type LessonProgress, type ModuleProgress, type ProgressSummary, type QuizAttempt } from '../db'
...
async function readLocalSnapshot(): Promise<ProgressSnapshot> {
  const [progress, moduleProgress, lessonProgress, quizAttempts, labProgress, examAttempts] = await Promise.all([db.progress.get('current'), db.moduleProgress.toArray(), db.lessonProgress.toArray(), db.quizAttempts.toArray(), db.labProgress.toArray(), db.examAttempts.toArray()])
  return { progress: { ...emptyProgress, ...progress }, moduleProgress, lessonProgress, quizAttempts, labProgress, examAttempts }
}
...
function mergeSnapshots(local: ProgressSnapshot, remote: ProgressSnapshot | null): ProgressSnapshot {
  ...
  const merged = { progress: emptyProgress, moduleProgress, lessonProgress, quizAttempts, labProgress, examAttempts }
...
function calculateSummary(snapshot: Omit<ProgressSnapshot, 'progress'> | ProgressSnapshot): ProgressSummary {
  const activityDates = [...snapshot.lessonProgress.map(item => item.completedAt), ...snapshot.quizAttempts.map(item => item.completedAt), ...snapshot.labProgress.map(item => item.completedAt), ...snapshot.examAttempts.map(item => item.completedAt)].map(value => value.slice(0, 10)).sort()
...
async function writeLocalSnapshot(snapshot: ProgressSnapshot) {
  await db.transaction('rw', [db.progress, db.moduleProgress, db.lessonProgress, db.quizAttempts, db.labProgress, db.examAttempts], async () => {
    await Promise.all([db.progress.clear(), db.moduleProgress.clear(), db.lessonProgress.clear(), db.quizAttempts.clear(), db.labProgress.clear(), db.examAttempts.clear()])
