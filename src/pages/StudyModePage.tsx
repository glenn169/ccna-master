...
import { answerInstruction, canonicalAnswerIndexes, correctAnswerIndexes, isQuestionCorrect, questionsByTopic, randomizeQuestion, shuffleItems, type PracticeQuestion } from '../questions'
...
export function StudyModePage() {
  ...
  function poolFor(selectedMode: StudyMode) {
    ...
    return [...shuffleItems(pool.filter((item) => !recentIds.has(item.id))), ...shuffleItems(pool.filter((item) => recentIds.has(item.id)))]
  }
  function start(selectedMode = mode) { const pool = poolFor(selectedMode); if (!pool.length) return; setMode(selectedMode); setQuestions(pool.slice(0, Math.min(count, pool.length)).map((question) => ({ ...question, ...randomizeQuestion(question) }))); setIndex(0); setAnswers({}); setScore(0); setState('active') }
